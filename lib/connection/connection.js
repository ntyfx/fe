...
class Connection {
    ...
    remote(command, options) {
        command.split(' && ').forEach(c => {
            gutil.log(`${this.label} 运行命令 ${chalk.cyan(c)}`);
        });
    ...
    generateScpCopyCommands(options, src, dest) {
        ...
        let rm2 = `if [ -f ${packageFile} ]; then ${rm}; else exit 0; fi`;
        let rmSource = [cdSource, rm2].join(' && ');
        let rmDest = [cdDest, rm2].join(' && ');
        ...
        /*
            ...
            6. remote:  删除目标 tar
         */
        const fromPathLabel = chalk.magenta(fromPath);
        const toPathLabel = chalk.blue(`${toPath}/${packageFile}`);
        return [{
            desc: `${this.label} 创建远程目录 ${chalk.blue(toPath)}`,
            cmd: mkdir
        }, {
            desc: `${this.label} 打包文件 ${fromPathLabel}`,
            cmd: tar
        }, {
            desc: `${this.label} 上传文件 ${fromPathLabel}`,
            cmd: scp
        }, {
            desc: `${this.label} 远程解压 ${toPathLabel}`,
            cmd: untar
        }, {
            desc: `${this.label} 删除远程临时文件 ${toPathLabel}`,
            cmd: rmDest
        }, {
            desc: `${this.label} 删除本地临时文件 ${fromPathLabel}`,
            cmd: rmSource
        }];
    }
